...
const ListingDetails = () => {
  ...
  const createRental = async () => {
    ...
    try {
      // Generate a rental code for this transaction
      const rentalCodeGenerated = `BBR-${Math.random().toString(36).substring(2, 6).toUpperCase()}`;
      ...
      const { data: rentalData, error } = await supabase
        ...
        .insert({
          ...
          rental_code: rentalCodeGenerated
        })
        .select()
        .single();
      ...
      // Get the rental code from the response
      if (rentalData) {
        setRentalCode(rentalData.rental_code);
        setShowPaymentDialog(true);
        
        // Update revenue for the seller
        const sellerPayout = totalPrice * 0.85; // 85% of total price
        
        // We'll handle revenue updates in the UI directly, 
        // but in a real app this would be done server-side
      } else {
